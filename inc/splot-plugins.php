	...
	function __construct() {
		...
		$this->plugins = array(
			...
			array(
				...
				'message' => sprintf(
					esc_html__( 'The %1$s is recommended to hide the dashboard from the special writer user account used to guest author on this site.', 'splotbox' ),
					'<strong>' . esc_html__( 'Remove Dashboard Access plugin', 'splotbox' ) . '</strong>' ),
			),
			
			array(
				'slug'    => 'fluidvids',
				'name'    => 'FluidVids for WordPress',
				'message' => sprintf(
					esc_html__( 'The %1$s is useful for making sure embedded videos are displayed responsively', 'splotbox' ),
					'<strong>' . esc_html__( 'FluidVids for WordPress plugin', 'splotbox' ) . '</strong>' ),
			),
	...
	function admin_notices() {
		...
		foreach ( $this->plugins as $key => $plugin ) {
			...
			if ( 'to-activate' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					esc_html__( ' To do this, activate %1$s. %2$s', 'splotbox' ),
					esc_html( $plugin['name'] ),
					( $activate_url ) ? '<a href="' . $activate_url . '">' . esc_html__( 'Activate Now', 'splotbox' ) . '</a>' : ''
				);
			}
			...
			if ( 'to-install' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					esc_html__( ' To do this, install %1$s. %2$s', 'splotbox' ),
					esc_html( $plugin['name'] ),
					( $install_url ) ? '<a href="' . $install_url . '">' . esc_html__( 'Install Now', 'splotbox' ) . '</a>' : ''
				);
			}
...
function splot_dismiss_admin_notice() {
	...
	if ( ! isset( $_POST['splot_admin_nonce'] ) || ! wp_verify_nonce( $_POST['splot_admin_nonce'], 'splot_admin_nonce' ) ) {
		wp_die( __( 'Your request failed permission check.', 'splotbox' ) );
	}
	...
	wp_send_json( array(
		'status' => 'success',
		'message' => __( 'Your request was processed. See ya!', 'splotbox' )
	) );
}
